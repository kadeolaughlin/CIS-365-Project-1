...
class ReflexCaptureAgent(CaptureAgent):
  ...
  def chooseAction(self, gameState):
    ...
    for action in bestActions:
      if self.getFeatures(gameState, a)['fleeEnemy'] <= 3.0 and gameState.getAgentState(self.index).isPacman:
        ghostClose = True

    if ghostClose or gameState.getAgentState(self.index).numCarrying >= 3:
      shortestHomeDist = self.getMazeDistance(gameState.getAgentPosition(self.index),self.start)
      ...
      for action in actions:     
        ...
        if dist < bestDist and (ghostDistance2 - ghostDistance1) >= - 1.0:
          print("Dist")
          print(dist)

          print("Difference of ghost distances")
          print(ghostDistance2 - ghostDistance1)
          bestAction = action
          bestDist = dist        
      return bestAction        
